   ...
   procedure MAKEEWDS is 
      ...
      procedure EXTRACT_WORDS (S : in STRING;
         ...
         SEMI, COMMA : MEANING_TYPE := NULL_MEANING_TYPE;
...
--PUT_LINE("SEMI loop Processed");
       ...
       exception
          when others =>
            if (S(S'LAST) /= ')') or  (S(S'LAST) /= ']')  then    --  KLUDGE
               NEW_LINE;
               PUT_LINE("Extract Exception  LINE = " & 
 ...
 if DE.PART.POFS = V  and then
   ...
   null;
            ...
            --  Extract words
            EXTRACT_WORDS(TRIM(DE.MEAN), DE.PART.POFS, N, EWA); 
