...
--end loop;            
    for I in SL'FIRST..SL_LAST  loop
--TEXT_IO.PUT_LINE("SCANNING FOR TRIM   I = " & INTEGER'IMAGE(I));
    if SL(I).D_K in GENERAL..UNIQUE  then
      if (SL(I).IR.AGE =X) or (SL(I).IR.AGE > A)  then
         NOT_ONLY_ARCHAIC := TRUE;
       elsif SL(I).IR.AGE < F  then     --  Or E????
         NOT_ONLY_MEDIEVAL := TRUE;
       end if;
       if SL(I).IR.FREQ < C then  --  A/X < C   --  C for inflections is uncommon  !!!!
 ...
 --TEXT_IO.PUT_LINE("Uncommon       SL_LAST = " & INTEGER'IMAGE(SL_LAST) & "  I = " & INTEGER'IMAGE(I));
     ...
     if (SL(I).IR.QUAL.POFS = N)  then 
       if NOT_ONLY_VOCATIVE    and then 
        ...
        ((DEPR(SL(I)).KIND /= (N, N)) and (DEPR(SL(I)).KIND /= (N, P))) then
--TEXT_IO.PUT_LINE("N VOC not a P or N           "); 
         SL(I..SL_LAST-1) := SL(I+1..SL_LAST);
         SL_LAST := SL_LAST - 1;
         TRIMMED := TRUE;
       elsif NOT_ONLY_LOCATIVE    and then 
         (SL(I).IR.QUAL.N.CS = LOC) and then 
        ((DEPR(SL(I)).KIND /= (N, L)) and (DEPR(SL(I)).KIND /= (N, W))) then
